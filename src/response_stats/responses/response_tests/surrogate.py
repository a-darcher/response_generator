...
class SurrogateTest(ResponseTest):
    ...
    def binwise_test(self, baseline_hist, response_hist):
        ...
        pvals = np.empty(n_bins)
        tstats = np.empty(n_bins)
        for n in np.arange(n_bins):
            pval, tstat = ttest_ind(response_hist, baseline_hist, **self.ttest_kwargs)
            pvals[n] = pval
            tstat[n] = tstat
    ...
    def compute_empirical_tstat(self, data: ResponseData) -> float:
        ...
        _, binwise_tstats = self.binwise_test(baseline_hist, response_hist)
    ...
    def permute_tstats(self, data: ResponseData, surrogates: np.ndarray) -> np.array:
        ...
        baseline_ind = np.where(bins_whole_trial == data.cfg.baseline_T)[0]

        for n, s in enumerate(surrogates):
            b_hist = s[0:baseline_ind]
            r_hist = s[baseline_ind:]
            assert len(b_hist) + len(r_hist) == len(bins_whole_trial)

            _, b_tstats = self.binwise_test(b_hist, r_hist)
